      ...
      <div className="px-8 py-7 max-w-5xl">
        <div className="mb-7">
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            ...
            <a href="#" className="ml-2 text-indigo-400 hover:text-indigo-300">API docs →</a>
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 mb-5">
            ...
            {CATEGORIES.map(c => (
              <button key={c} onClick={() => setCategory(c)}
                className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors ${
                  category === c ? 'bg-indigo-500/20 text-indigo-400' : 'text-slate-500 hover:text-slate-300'
                }`}>
                {c}
              </button>
            ))}
          ...
          <div className="relative">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…"
              className="pl-8 pr-3 py-1.5 bg-[#12141f] border border-[#1f2235] rounded-lg text-xs text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500 w-40" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {filtered.map(integration => (
            <div key={integration.id}
              className="bg-[#12141f] rounded-xl border border-[#1f2235] p-5 hover:border-[#2a2d3a] transition-colors">
              <div className="flex items-start gap-3 mb-3">
                <div className="text-2xl w-10 h-10 rounded-xl bg-[#0d0f18] flex items-center justify-center shrink-0">
                  {integration.logo}
                </div>
              ...
              {integration.status === 'coming_soon' && (
                <p className="text-xs text-amber-400/70 mt-3">Native connector coming soon — use webhooks in the meantime.</p>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="mt-6 bg-[#12141f] rounded-xl border border-[#1f2235] p-5">
          <h2 className="text-sm font-medium text-white mb-2">Using the Webhook API</h2>
          <p className="text-xs text-slate-400 leading-relaxed mb-3">
            All integrations can be powered by Vani's webhook system. Configure webhooks at Settings → Webhooks to receive real-time event payloads for <code className="text-indigo-300 bg-indigo-500/10 px-1 rounded">call.started</code>, <code className="text-indigo-300 bg-indigo-500/10 px-1 rounded">call.ended</code>, and <code className="text-indigo-300 bg-indigo-500/10 px-1 rounded">call.analyzed</code> events.
          </p>
          <a href="/webhooks" className="text-xs text-indigo-400 hover:text-indigo-300 transition-colors">
            Configure webhooks →
          </a>
        </div>
      </div>
