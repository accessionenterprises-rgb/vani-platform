      ...
      <div className="px-8 py-7 max-w-5xl">
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-[#12141f] rounded-xl border border-[#1f2235]">
            {search ? (
              <>
                <p className="text-slate-400 font-medium mb-1">No agents match "{search}"</p>
                <button onClick={() => setSearch('')} className="text-xs text-indigo-400 hover:text-indigo-300 mt-2">Clear search</button>
              </>
            ) : (
              <>
          ...
          <>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-4">
                <p className="text-xs text-slate-500">{filtered.length} agents</p>
                <div className="flex items-center gap-2">
